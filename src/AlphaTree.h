...
class AlphaTreeWrapper
{
	void *tree;
	int imgidx, pixel;
public:
	AlphaTreeWrapper() : tree(0) {}
	~AlphaTreeWrapper() { if (tree) { Free(tree);} }
	inline void clear()
	{
/*		if(tree)
		{
	...
	*/
	...
	void BuildAlphaTree(uint8 *img, int height, int width, int channel, int connectivity, int par)
	{
		...
		if((int64)height * (int64)width < (int64)0xefffffff)
		{
			...
			tree = Malloc(sizeof(AlphaTree<int32,uint8>));
			((AlphaTree<int32,uint8>*)tree)->BuildAlphaTree(img, height, width, channel, connectivity, par);
		}
		else
		{
			imgidx = IMGIDX_64BITS;
			tree = Malloc(sizeof(AlphaTree<int64,uint8>));
			((AlphaTree<int64,uint8>*)tree)->BuildAlphaTree(img, height, width, channel, connectivity, par);
		}
	}
	...
	void BuildAlphaTree(uint16 *img, int height, int width, int channel, int connectivity, int par)
	{
		...
		{
			imgidx = IMGIDX_32BITS;
			tree = Malloc(sizeof(AlphaTree<int32,uint16>));
			((AlphaTree<int32,uint16>*)tree)->BuildAlphaTree(img, height, width, channel, connectivity, par);
		}
		else
		{
			imgidx = IMGIDX_64BITS;
			tree = Malloc(sizeof(AlphaTree<int64,uint16>));
			((AlphaTree<int64,uint16>*)tree)->BuildAlphaTree(img, height, width, channel, connectivity, par);
		}
	}
	void BuildAlphaTree(uint32 *img, int height, int width, int channel, int connectivity, int par)
	{
		...
		{
			imgidx = IMGIDX_32BITS;
			tree = Malloc(sizeof(AlphaTree<int32,uint32>));
			((AlphaTree<int32,uint32>*)tree)->BuildAlphaTree(img, height, width, channel, connectivity, par);
		}
		else
		{
			imgidx = IMGIDX_64BITS;
			tree = Malloc(sizeof(AlphaTree<int64,uint32>));
			((AlphaTree<int64,uint32>*)tree)->BuildAlphaTree(img, height, width, channel, connectivity, par);
		}
	}
	void BuildAlphaTree(uint64 *img, int height, int width, int channel, int connectivity, int par)
	{
		...
		{
			imgidx = IMGIDX_32BITS;
			tree = Malloc(sizeof(AlphaTree<int32,uint64>));
			((AlphaTree<int32,uint64>*)tree)->BuildAlphaTree(img, height, width, channel, connectivity, par);
		}
		else
		{
			imgidx = IMGIDX_64BITS;
			tree = Malloc(sizeof(AlphaTree<int64,uint64>));
			((AlphaTree<int64,uint64>*)tree)->BuildAlphaTree(img, height, width, channel, connectivity, par);
		}
	}
	...
	void BuildAlphaTree(float *img, int height, int width, int channel, int connectivity, int par)
	{
		...
		if((int64)height * (int64)width < (int64)0xefffffff)
		{
			imgidx = IMGIDX_32BITS;
			tree = Malloc(sizeof(AlphaTree<int32,uint32>));
			((AlphaTree<int32,uint32>*)tree)->BuildAlphaTree((uint32*)img, height, width, channel, connectivity, par);
		}
		else
		{
			imgidx = IMGIDX_64BITS;
			tree = Malloc(sizeof(AlphaTree<int64,uint32>));
			((AlphaTree<int64,uint32>*)tree)->BuildAlphaTree((uint32*)img, height, width, channel, connectivity, par);
		}
	}
	...
	void BuildAlphaTree(double *img, int height, int width, int channel, int connectivity, int par)
	{
		...
		if((int64)height * (int64)width < (int64)0xefffffff)
		{
			imgidx = IMGIDX_32BITS;
			tree = Malloc(sizeof(AlphaTree<int32,uint64>));
			((AlphaTree<int32,uint64>*)tree)->BuildAlphaTree((uint64*)img, height, width, channel, connectivity, par);
		}
		else
		{
			imgidx = IMGIDX_64BITS;
			tree = Malloc(sizeof(AlphaTree<int64,uint64>));
			((AlphaTree<int64,uint64>*)tree)->BuildAlphaTree((uint64*)img, height, width, channel, connectivity, par);
		}
	}
	/*
	void BuildAlphaTree(float *img, int height, int width, int channel, int connectivity, int par)
	{
		...
		{
			imgidx = IMGIDX_32BITS;
			tree = Malloc(sizeof(AlphaTree<int32,float>));
			((AlphaTree<int32,float>*)tree)->BuildAlphaTree(img, height, width, channel, connectivity, par);
		}
		else
		{
			imgidx = IMGIDX_64BITS;
			tree = Malloc(sizeof(AlphaTree<int64,float>));
			((AlphaTree<int64,float>*)tree)->BuildAlphaTree(img, height, width, channel, connectivity, par);
		}
	}
	void BuildAlphaTree(double *img, int height, int width, int channel, int connectivity, int par)
	{
		...
		{
			imgidx = IMGIDX_32BITS;
			tree = Malloc(sizeof(AlphaTree<int32,double>));
			((AlphaTree<int32,double>*)tree)->BuildAlphaTree(img, height, width, channel, connectivity, par);
		}
		else
		{
			imgidx = IMGIDX_64BITS;
			tree = Malloc(sizeof(AlphaTree<int64,double>));
			((AlphaTree<int64,double>*)tree)->BuildAlphaTree(img, height, width, channel, connectivity, par);
		}
	}
	*/
};
